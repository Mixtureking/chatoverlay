/**
 * createApiApp.ts — Pure Express API app factory.
 *
 * This module is shared by BOTH:
 *   - server.ts   (local dev / Electron / standalone production)
 *   - api/index.ts (Vercel serverless function)
 *
 * It intentionally does NOT import `vite`, `fs`, `path`, or any native-only
 * module that would crash or bloat a Vercel Lambda bundle.
 */
...
// Lightweight logger — console only (no fs). File logging is added by server.ts for local dev.
...
/**
 * Creates a fully configured Express application with all API routes.
 * Does NOT include Vite middleware, static file serving, or app.listen().
 */
export function createApiApp(): express.Express {
  ...
  // ─── API Route 1: Get Active Live Chat ID & Broadcast Info ───
  app.post(["/api/youtube/live-chat-id", "/youtube/live-chat-id", "/live-chat-id", "*/live-chat-id"], async (req, res): Promise<any> => {
    try {
      ...
      log(`Nhận yêu cầu kết cấu /api/youtube/live-chat-id. Video input: "${videoUrlOrId}", API Key ẩn: "${apiKey && typeof apiKey === 'string' ? apiKey.substring(0, 6) + '...' : 'Không có'}"`);

      if (!videoUrlOrId) {
        log("Lỗi: Thiếu thông tin Video URL hoặc ID");
      ...
      log(`Trích xuất thành công Video ID: "${videoId}"`);
      ...
      log(`Google API trả về mã phản hồi: ${response.status} (${response.statusText})`);

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        const message = errorData?.error?.message || "Lỗi API từ Google";
        const status = response.status;
        log(`Lỗi kết nối hoặc API phản hồi không thành công: ${JSON.stringify(errorData)}`);
        return res.status(status).json({ error: `Google API Error (${status}): ${message}` });
      }

      const data = await response.json();
      log(`Dữ liệu Google API lấy về thành công: items count = ${data.items ? data.items.length : 0}`);
      if (!data.items || data.items.length === 0) {
        log(`Lỗi: Không tìm thấy video trên YouTube với ID "${videoId}"`);
      ...
      if (!liveStreamingDetails) {
        log(`Lỗi: Video "${videoId}" không chứa liveStreamingDetails. Không phải livestream/trực tiếp.`);
      ...
      log(`Thông tin livestream: activeLiveChatId="${activeLiveChatId}"`);
      if (!activeLiveChatId) {
        log(`Lỗi: Livestream "${videoId}" không có activeLiveChatId. Có thể livestream đã kết thúc.`);
        return res.status(400).json({
          error: "Livestream này đã kết thúc hoặc không có khung chat trực tiếp nào đang hoạt động."
        });
      }

      log(`Thực hiện trả về dữ liệu kết nối thành công: activeLiveChatId="${activeLiveChatId}", title="${snippet.title}"`);
      res.json({
        activeLiveChatId,
        videoId,
        title: snippet.title,
      ...
      log(`Gặp exception cực kỳ nghiêm trọng trong live-chat-id try-catch: ${error?.message || error}\nStack trace: ${error?.stack}`);
    ...
    try {
      ...
      const messages = rawItems.map((item: any) => {
        ...
        if (isSuperChat && snippet.superChatDetails) {
          ...
          if (amountVal >= 100) { tier = 6; superChatColor = "#e91e63"; }
          else if (amountVal >= 50) { tier = 5; superChatColor = "#e65100"; }
          else if (amountVal >= 20) { tier = 4; superChatColor = "#fdd835"; }
          else if (amountVal >= 10) { tier = 3; superChatColor = "#00e676"; }
          else if (amountVal >= 5) { tier = 2; superChatColor = "#00b0ff"; }
          else { tier = 1; superChatColor = "#1565c0"; }
  ...
  // ─── Server-side settings cache (OBS ↔ Dashboard sync) ───
