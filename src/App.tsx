...
const SAMPLE_MESSAGES_TEMPLATES = [
  ...
  { name: "Người Xem A", text: "!vote A", role: "normal" },
  { name: "Người Xem B", text: "!vote B", role: "normal" },
  { name: "Fan Cứng", text: "!vote A", role: "sponsor" },
];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const isSprint7Route = path.endsWith("/obs-chat") || path.endsWith("/obs-timer") || path.endsWith("/obs-wheel") || path.endsWith("/obs-link") || path.endsWith("/obs-todo");
  ...
  const handleInjectMessage = (persona: typeof SAMPLE_MESSAGES_TEMPLATES[0]) => {
    ...
    // Automatically trigger vote interactivity for simulated/injected vote commands
    const msgTextTrim = persona.text.trim();
    if (msgTextTrim.startsWith("!")) {
      const match = msgTextTrim.match(/^!vote\s+([AB])$/i);
      if (match) {
        const option = match[1].toUpperCase();
        fetch("/api/interactivity/votes", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId: persona.name, option })
        }).catch(err => console.error("Simulated vote casting error:", err));
      }
    }
  };
  ...
  const sprint7Route = typeof window !== "undefined"
    ? (window.location.pathname.endsWith("/obs-chat") || window.location.pathname.endsWith("/obs-timer") || window.location.pathname.endsWith("/obs-wheel") || window.location.pathname.endsWith("/obs-link") || window.location.pathname.endsWith("/obs-todo"))
